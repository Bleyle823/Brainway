...
export function getRecallBridgeUrl(): string {
  ...
  if (!raw) {
    throw new Error(
      "RECALL_BRIDGE_URL is not configured. Deploy recall-bridge (see recall-bridge/README.md at the repo root), set PUBLIC_URL there, then add RECALL_BRIDGE_URL in Brainway.",
    );
  }
  return raw.replace(/\/+$/, "");
}
